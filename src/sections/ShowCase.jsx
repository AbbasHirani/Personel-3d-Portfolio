...
import { proj } from '../constants/index.js';
...
const ShowCase = () => {
  ...
  useGSAP(() => {
    // Reset refs only inside the effect
  ...
  return (
    <section id="work" className="app-showcase" ref={section1Ref}>
      <div className="w-full">
        <h2 className="text-4xl md:text-6xl font-semibold text-white mb-6 "> Projects </h2>
        {proj.map((project, index) => (
          <div
            key={project.id || index}
            className="mt-1 mb-2 bg-zinc-900 px-6 pt-1 rounded-xl mb-15"
            ref={addToRefs}
          >
            <div className="mt-5 mb-6 text-center">
              <h2 className="text-2xl md:text-3xl font-semibold text-white">{project.projectName}</h2>
            </div>
            <div className="showcaselayout pb-5">
              {/* left side main Image */}
              <div className="first-project-wrapper" ref={addToRefs}>
                <div className="image-wrapper">
                  <img src={project.img1Path} alt="Proj-img-1" />
                </div>
                <div className="text-content">
                  {/* decription */}
                  <div className="max-w-full  bg-opacity-30 px-3 pt-2 pb-2 rounded-lg">
                    <div className="max-w-full">
                      {project.img1desc.map((para, idx) => 
                        Object.values(para).map((para, idx) =>(
                        <p className="text-white text-opacity-80 sm:text-sm md:text-base lg:text-lg leading-relaxed mb-3" key={idx}>
                          {para}
                        </p>
                      )))}
                    </div>
                  ...
                  <div className="w-full flex flex-wrap gap-3 mt-3 items-center px-3">
                    ...
                    {project.techstack.map((tech, i) => (
                      <div key={tech.name || i} className="flex items-center gap-1 px-2 py-1 rounded bg-white/10 text-xs">
                        <img src={tech.imgPath} alt={tech.name} className="w-4 h-4" />
                        <span>{tech.name}</span>
                      </div>
                    ))}
                  </div>
              ...
              <div className="project-list-wrapper overflow-hidden ">
                <div className="project" ref={addToRefs}>
                  <div className="image-wrapper bg-[#ffe7eb]">
                    <img src= {project.img2Path} alt="Project-2" />
                  </div >
                  <p className='pt-3 pl-1'>{project.img2desc}</p>
                </div>
                <div className="project" ref={addToRefs}>
                  <div className="image-wrapper bg-[#ffefdb]">
                    <img src= {project.img3Path} alt="Project-3" />
                  </div>
                  <p className='pt-3 pl-1'>{project.img3desc}</p>
                </div>
              </div>
            </div>
          </div>
